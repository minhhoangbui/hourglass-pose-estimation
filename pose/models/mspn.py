...
class UpsampleUnit(nn.Module):
    ...
    def __init__(self, ind, in_planes, up_size, output_chl_num, output_shape,
        ...
        self.u_skip = ConvBNReLu(in_planes, chl_num, kernel_size=1, stride=1,
                                 padding=0, has_bn=True, has_relu=False,
                                 efficient=efficient)
        self.relu = nn.ReLU(inplace=True)
    ...
    def forward(self, x, up_x):
        ...
        if self.ind > 0:
            up_x = F.interpolate(up_x, size=self.up_size, mode='nearest',
                                 align_corners=True)
        ...
        res = F.interpolate(res, size=self.output_shape, mode='nearest',
                            align_corners=True)
...
class Upsample(nn.Module):
    ...
    def __init__(self, output_chl_num, output_shape, chl_num=256,
        ...
        self.up1 = UpsampleUnit(0, self.in_planes[0], self.up_sizes[0],
        ...
        self.up2 = UpsampleUnit(1, self.in_planes[1], self.up_sizes[1],
                                output_chl_num=output_chl_num, output_shape=output_shape,
                                chl_num=chl_num, gen_skip=self.gen_skip,
                                gen_cross_conv=self.gen_cross_conv, efficient=efficient)
        self.up3 = UpsampleUnit(2, self.in_planes[2], self.up_sizes[2],
                                output_chl_num=output_chl_num, output_shape=output_shape,
                                chl_num=chl_num, gen_skip=self.gen_skip,
                                gen_cross_conv=self.gen_cross_conv, efficient=efficient)
        self.up4 = UpsampleUnit(3, self.in_planes[3], self.up_sizes[3],
                                output_chl_num=output_chl_num, output_shape=output_shape,
                                chl_num=chl_num, gen_skip=self.gen_skip,
                                gen_cross_conv=self.gen_cross_conv, efficient=efficient)
